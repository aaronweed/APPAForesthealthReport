...
mort_year_top10 %>%
  ggplot(aes(x = MEASYEAR, y = GROW_TOTAL)) +
  geom_col() +
  facet_wrap(~SCIENTIFIC_NAME) +
  theme_bw()
...
mort_year_top10 %>%
  ...
  summarize(Average_Mortality_Percent = mean(MORT_PERC))


# choropleth map for diameter growth rate (in/yr)
pal <- colorBin("Purples", domain = mort[["MORT_TOTAL"]], bins = 9)

mort %>%
  leaflet() %>%
  addTiles() %>%
  addPolygons(fillColor = ~ pal(mort[["MORT_TOTAL"]]),
              fillOpacity = 1,
              opacity = 0,
              popup = paste0("<b>Ecosubsection: </b>",
                             mort[["SUBSECTION"]],
                             "<br> <b>mean Shannon's Equitability Index,<br> alpha (stand) level: </b>",
                             round(mort[["MORT_TOTAL"]], 3))) %>%
  addPolylines(data = at_centerline,
               color = "black",
               opacity = 1,
               weight = 2) %>%
  addLegend(pal = pal,
            values = ~mort[["MORT_TOTAL"]],
            title = "mean Shannon's Equitability Index,<br> alpha (stand) level",
            position = "bottomright") 
