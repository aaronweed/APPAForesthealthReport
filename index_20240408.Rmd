...
```{r report-load-path, include=FALSE}
summary_tables <- "C:/Users/JJGross/Documents/R_projects/FIA_data/summary_data/most_recent/"

support_tables <- "C:/Users/JJGross/Documents/R_projects/FIA_data/clip_FIA/"

```

```{r support-tables, include=FALSE}
load_location <- support_tables

# AT centerline shapefile
at_centerline <- readRDS(paste0(load_location, "at_centerline.rds")) 

n_plots_per_ecosubsection <- readRDS(paste0(load_location, "subsection_attributes.rds")) |>
  st_as_sf()

plt_loc<-readRDS(paste0(load_location, "subsection_centroids.rds"))# points layer of plot locations

# center points of Ecological Section for SetView
centroids <- readRDS(paste0(load_location, "section_centroids.rds"))

# elevation of plots
elevs_yr<- readRDS(paste0(load_location, "plot_attributes_by_year.rds"))
samp_years<-elevs_yr %>% 
  sf::st_drop_geometry() %>%
  filter(SECTION_NA %in% params$section) %>% # filter data by Section
  ungroup() %>% 
  summarise(min= min(YEAR), max= max(YEAR)) #find first and last years of sampling
```

```{r summary-data, include=FALSE}

load_location <- summary_tables

diversity <- readRDS(paste0(load_location, "diversity.rds"))
#div_year <- readRDS(paste0(path_to_summary_data, "div_year.rds"))

ss <-readRDS(paste0(load_location, "ss.rds")) # Stand Structure

snag <- readRDS(paste0(load_location, "snag_abundance.rds")) # Snag density trees >5"DBH

downwoody <- readRDS(paste0(load_location, "downwoody.rds"))

all_tpa <-readRDS(paste0(load_location, "dead_and_live_tpa.rds")) # density of live and standing dead trees

tpa_live<-readRDS(paste0(load_location, "tpa.rds")) # density of live trees (>5" DBH)
tpa_Spp<- readRDS(paste0(load_location, "tpa_spp.rds")) # density of tree species by species
tpa_Spp_SizeCl<- readRDS(paste0(load_location, "tpa_spp_sizecl.rds")) # density of tree species by species and size class

saplings<-readRDS(paste0(load_location, "saplings.rds")) |> 
  add_column(variable = "Saplings (1-5 in. DBH)")

saplings_Spp<-readRDS(paste0(load_location, "saplings_spp.rds")) %>% 
  add_column(variable = "Saplings (1-5 in. DBH)")

seedlings<-readRDS(paste0(load_location, "seedlings.rds")) %>% 
  add_column(variable = "Seedlings (<1 in. DBH)")

seedlingsSpp<-readRDS(paste0(load_location, "seedlings_spp.rds")) %>% 
  add_column(variable = "Seedlings(<1 in. DBH)")

mort <- readRDS(paste0(load_location, "mortality.rds")) # mortality rates by Species
growth<- readRDS(paste0(load_location, "growth.rds")) # DBH growth rates by Species

invasives <- readRDS(paste0(load_location, "invasive.rds"))

```

::: {.alert .alert-danger}
Not created in 03-make.Rmd because `rFIA::biomass` function not working:

**biomass.rds**, **live_vol.rds**, **snagV.rds**

temporary solution - manually downloaded these tables from Aaron's sharepoint folder.
:::

```{r aarons-sharepoint-folder, include=FALSE}
aarons_sharepoint <- "C:/Users/JJGross/Documents/R_projects/FIA_data/from_aarons_sharepoint/"
```

```{r missing-rFIA-errors, include=FALSE}
live_bio <- readRDS(paste0(aarons_sharepoint, "biomass.rds")) # live tree biomass
live_vol<- readRDS(paste0(aarons_sharepoint, "live_vol.rds")) # live tree volume
snagV <- readRDS(paste0(aarons_sharepoint, "snagV.rds")) # Snag volume trees >5"DBH
```
...
pal <- colorBin("Oranges", domain = n_plots_per_ecosubsection[["n_plots_subsect"]], bins = 5, na.color = NA )
...
n_plots_per_ecosubsection %>% 
  ...
  addTiles() %>%
  addPolygons(fillColor = ~ pal(n_plots_per_ecosubsection[["n_plots_subsect"]]),
              fillOpacity = 1,
              opacity = 1,
              ...
              popup = paste0("<b>Ecological Section: </b>",
                             ...
                             n_plots_per_ecosubsection[["n_plots_subsect"]])) %>%
  ...
  addPolylines(data = at_centerline,
               ...
               weight = 2) %>%
  addLegend(pal = pal,
            values = ~n_plots_per_ecosubsection[["n_plots_subsect"]],
...
Map of Appalachian Trail HUC10 Shell and AT center line showing number of FIA plots per Ecoregional SubSections (Cleland et al. 2007). Clicking on the subsection will display the number of FIA plots used to summarize forest health condition.
...
elevs_yr %>%
  ...
  group_by(SUBSECTI_1) %>% 
  summarise(`No. FIA plots`= n(), 
            `Average Elevation (ft)`= mean(ELEV), 
            `Elevation Range (ft)` = paste0(min(ELEV)," - ", max(ELEV))) %>% 
  select(`Ecol. Subsection` = SUBSECTI_1, 
         `No. FIA plots`, 
         `Average Elevation (ft)`, 
         `Elevation Range (ft)`)  %>%
  kableExtra::kbl(
    .,digits = 1, escape = FALSE, align= c("l", "c","c","c"), longtable= T, 
    caption= paste0("Total number and elevation of FIA plots in each subsection sampled between",
                    samp_years$min, " - ",samp_years$max,"." )) %>%
...
elevs_yr %>% 
  ...
  summarise(`No. FIA plots`= n(), 
            ...
            `Elevation Range (ft)` = paste0(min(ELEV)," - ", max(ELEV))) %>% 
  select(`Ecol. Subsection` = SUBSECTI_1, 
         Year= YEAR,`No. FIA plots`, 
         `Average Elevation (ft)`, 
         `Elevation Range (ft)`) %>% 
  kableExtra::kbl(
    ., digits = 1, escape = FALSE, align= c("l", "c","c","c","c"), longtable= T, 
    caption= paste0("Total number and elevation of FIA plots in each subsection sampled each year.")) %>%
...
::: {.alert .alert-danger}
Should Perc_Snags be Dead / (Dead + Live) instead of Dead/Live?
:::
...
Perc_Snags<-all_tpa %>% 
  ...
  group_by(SECTION_NA, SUBSECTI_1, tree_status) %>% 
  summarise(Trees = sum (Trees)) %>%
  pivot_wider(id_cols = c("SECTION_NA","SUBSECTI_1"), names_from = "tree_status", values_from = "Trees") %>% 
...
Perc_Snags %>%
  st_drop_geometry() |>
  left_join (Perc_ML_Snags, by = c("SUBSECTI_1")) %>% 
  select(Subsection= SUBSECTI_1, 
         `% of Trees as Snags`= Perc_Snags, 
         `% of Trees as M-L Snags (>30 cm DBH)` = Perc_ML_Snags) %>% 
  kableExtra::kbl(.,digits = 2, escape = FALSE, align= c("l","c","c"), longtable= T, 
                  caption= "Snag abundance per subsection. Metric ratings are based on the percent of standing trees (i.e., ≥ 5 in. diameter at breast height) that are snags. M-L Snags refers to medium to large (≥ 11.8 in. diameter at breast height) diameter snags.") %>%   
  kable_styling(fixed_thead = TRUE, bootstrap_options = c("striped", "hover", "condensed", "responsive")) 
...
tpa_Spp_SizeCl$newClass <- rFIA::makeClasses(tpa_Spp_SizeCl$sizeClass, interval = 5)
...
::: {.alert .alert-danger}
Questioning if column `N` is the correct number of plots for calculating SE below in growth table `sqrt(DIA_GROW_VAR)/sqrt(N)` - maybe `nPlots_TREE` is correct N?
:::
...
::: {.alert .alert-danger}
Questioning if column `N` is the correct number of plots for calculating SE below in mortality table `sqrt(MORT_PERC_VAR)/sqrt(N)` - maybe `nPlots_TREE` is correct N?
:::
...
::: {.alert .alert-danger}
Questioning if column `N` is the correct number of plots for calculating SE below in growth table `sqrt(COVER_PCT_VAR)/sqrt(N)` - maybe `nPlots_INV` is correct N?
:::
...
invasives %>% as_tibble() %>% 
  ...
  select(SUBSECTI_1, Year= YEAR,COMMON_NAME,  COVER_PCT, SE)%>%   
...
invasives %>% as_tibble() %>% 
  ...
  select(`Ecol. Subsection` = SUBSECTI_1, Year= YEAR,`Common Name` = COMMON_NAME, `Latin Name`= SCIENTIFIC_NAME, `Percent Cover (SE)`, `Plots Present`= nPlots_INV, `Percent Plots Present`) %>% 
